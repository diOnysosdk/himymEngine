...
#endif
#ifndef HIMYM_USE_XM
#define HIMYM_USE_XM 1
#endif
#ifndef HIMYM_USE_PIXEL
#define HIMYM_USE_PIXEL 1
#endif
#ifndef HIMYM_USE_PARTICLES
#define HIMYM_USE_PARTICLES 1
#endif
#ifndef HIMYM_USE_MESH
#define HIMYM_USE_MESH 1
#endif
...
bool LoadRuntimePlaybackSettings(const char* path, RuntimePlaybackSettings* settings) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (sscanf_s(s, "total_duration=%f", &settings->total_duration) == 1) {
            ...
            found = true;
        } else if (sscanf_s(s, "audio_curve_gain_db=%d", &settings->audio_effects.curve_gain_db) == 1) {
            found = true;
            ...
            found = true;
        } else if (sscanf_s(s, "audio_curve_compressor_threshold=%d", &settings->audio_effects.curve_compressor_threshold) == 1) {
            found = true;
        } else if (sscanf_s(s, "audio_curve_compressor_ratio=%d", &settings->audio_effects.curve_compressor_ratio) == 1) {
            found = true;
        } else if (sscanf_s(s, "audio_curve_compressor_attack=%d", &settings->audio_effects.curve_compressor_attack) == 1) {
            found = true;
        } else if (sscanf_s(s, "audio_curve_compressor_release=%d", &settings->audio_effects.curve_compressor_release) == 1) {
            found = true;
            ...
            found = true;
        } else if (sscanf_s(s, "audio_curve_widener_amount=%d", &settings->audio_effects.curve_widener_amount) == 1) {
            found = true;
            ...
            found = true;
        } else if (sscanf_s(s, "audio_curve_eq_low_db=%d", &settings->audio_effects.curve_eq_low_db) == 1) {
            found = true;
        } else if (sscanf_s(s, "audio_curve_eq_mid_db=%d", &settings->audio_effects.curve_eq_mid_db) == 1) {
            found = true;
        } else if (sscanf_s(s, "audio_curve_eq_high_db=%d", &settings->audio_effects.curve_eq_high_db) == 1) {
            found = true;
        }
    }
...
static void BuildMeshProjection(float out[16], const rev::mesh::Mesh* mesh,
    ...
    if (!use_imported_camera || !mesh || !mesh->has_imported_camera) {
        Mat4Perspective(out, fallback_fov_deg * 3.14159265f / 180.0f,
        ...
        for (int wf = 0; wf < kWarmupFrames; ++wf) {
            ...
            if (mesh_shader && has_mesh) {
                ...
                for (int mi = 0; mi < mesh_cue_count; ++mi) {
                    ...
                    BuildMeshProjection(proj_mat, mesh_obj,
                        mesh_cue.use_imported_camera != 0,
                        camera_fov_deg, aspect);
                    if (glUniformMatrix4fv_fn && loc_view >= 0) glUniformMatrix4fv_fn(loc_view, 1, 0, view_mat);
                    if (glUniformMatrix4fv_fn && loc_proj >= 0) glUniformMatrix4fv_fn(loc_proj, 1, 0, proj_mat);
                    ...
                    for (uint32_t si = 0; si < mesh_obj->material_slot_count; ++si) {
                        ...
                        SetMeshNoiseUniforms(mesh_shader, &slot);
                    ...
                    if (!rendered_slot) {
                        ...
                        SetMeshNoiseUniforms(mesh_shader, nullptr);
    ...
    while (rev::platform::PollEvents(window) && !window->should_close) {
        ...
        if (audio_state && loaded_music_player_count > 0) {
            ...
            if (active_music_index >= 0) {
                ...
                if (selected_player) {
                    rev::runtime::AudioEffects evaluated_audio = {};
                    rev::runtime::EvaluateAudioEffects(&playback_settings.audio_effects,
                                                       curves, curve_count, time,
                                                       &evaluated_audio);
                    rev::xm::SetAudioEffects(selected_player, &evaluated_audio);
        ...
        if (scene_fbo_ready && post_shader && glBindFramebuffer_rt) {
            ...
            for (int scene_index = 0; scene_index < scene_layer_post_effect_count; ++scene_index) {
                ...
                for (int i = 0; i < scene_effects.effect_count; ++i) {
                    ...
                    if (effect.type == kPostEffectFade && effect.curve_intensity < 0) {
                        effect_intensity = rev::runtime::ComputePostFadeIntensity(
                            effect_intensity, scene_time, effect.start_time, effect_end,
                            effect_threshold, effect_radius);
                    }
                    add_effect(layer_effects, effect.type,
                               effect_intensity, effect_threshold, effect_radius,
