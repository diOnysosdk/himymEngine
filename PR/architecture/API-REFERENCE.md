...
- glTF emitter attachment: an emitter may reference a mesh cue asset key and a Blender-declared attachment node name, with local XYZ offset and socket/default or explicit `+/-X`, `+/-Y`, `+/-Z` axis. Imported attachment nodes retain their animated glTF hierarchy transform; preview and runtime project the socket origin and axis into the existing 2D particle simulation. Missing references fall back to the emitter's authored screen position and direction.
