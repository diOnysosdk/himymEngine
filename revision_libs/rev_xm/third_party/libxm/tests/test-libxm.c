...
static void print_position(const xm_context_t* ctx) {
    ...
    uint8_t pat;
    uint8_t row;
    xm_get_position(ctx, &pot, &pat, &row, NULL);
...
static int channelpairs_eq(xm_context_t* ctx, bool swap_lr, bool left_only) {
    ...
    chans = xm_get_number_of_channels(ctx);
...
static int channelpairs_pitcheq(xm_context_t* ctx) {
    ...
    xm_get_playing_speed(ctx, &bpm, NULL);
    if(bpm != 32) return 1;
    while(!xm_get_loop_count(ctx)) {
        xm_generate_samples_unmixed(ctx, frames, 3750);
        for(i = 0; i < 2; ++i) {
            a = modal_interpeak_distance(frames+i,   3750, 4);
            b = modal_interpeak_distance(frames+i+2, 3750, 4);
...
static int pat0_pat1_eq(xm_context_t* ctx0) {
    ...
    /* Copy the context */
    buf = malloc(xm_dump_size(ctx0));
    if(buf == NULL) return 1;
    xm_dump_context(ctx0, buf);
    ctx1 = xm_restore_context(buf);
    xm_seek(ctx1, 1, 0, 0);
...
static uint16_t modal_interpeak_distance(const float* data, uint16_t count, uint16_t stride) {
    uint16_t* counts;
    uint16_t i, last_peak_idx, last_peak_count;

    if(count < 3) return 0;

    counts = malloc(sizeof(uint16_t) * count);
    memset(counts, 0, sizeof(uint16_t) * count);
