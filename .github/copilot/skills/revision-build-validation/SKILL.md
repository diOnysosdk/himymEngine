...
## When both targets need rebuilding
After any change to `rev_runtime.h` or `rev_runtime.cpp` (structs, parsers, Mat4 functions), rebuild BOTH:
```powershell
cmake --build build --config Release --target editor_app minimal_intro
```
...
- After changing cues.txt export format, always verify parser field counts in `rev_runtime.cpp` match.
- After changing `rev_pack.cpp`, rebuild `editor_app` before re-packing.
- After changing `rev_runtime.h` or `rev_runtime.cpp`, rebuild both `editor_app` and `minimal_intro`.
- After adding a new cue type (e.g., MeshCue), verify: ExportProject field count = LoadXxxCue field count.
